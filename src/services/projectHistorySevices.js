...
export const updateProjectHistoryData = async (id, data) => {
  try {
    ...
    const resp = await axios.put(
      `${process.env.REACT_APP_PUBLIC_API_URL}/ProjectHistory/EditProject/projectID?projectID=${projectID}`,
      data,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
...
export const removeProjectHistoryData = async (id) => {
  try {
    let token = localStorage.getItem("jobPortalUserToken");
    let projectID = id;
    const resp = await axios.delete(
      `${process.env.REACT_APP_PUBLIC_API_URL}ProjectHistory/DeleteProjectHistory/projectID?projectID=${projectID}`,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
